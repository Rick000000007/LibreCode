...
export class Layout {
  ...
  renderStatusBar(provider: string, model: string, gitBranch: string | null, tokenPct: number, cwd: string = process.cwd()): void {
    ...
    const wd = ` \x1B[36m\u{1F4C2} ${dir}\x1B[39m`;

    let statusLine = `${theme.bg}${theme.fg}`;
    statusLine += `${prov}${wd}${git}${tokens}`;
  ...
  renderWorkflowPanel(steps: Array<{ label: string; status: string; detail?: string }>): void {
    ...
    if (r.width <= 0 || r.height <= 0 || steps.length === 0) return;

    // Only show the most recent step
    const step = steps[steps.length - 1]!;

    this.tui.cursorTo(r.x, r.y);

    const spinnerFrames = ['\u280B', '\u2819', '\u2839', '\u2838', '\u283C', '\u2834', '\u2826', '\u2827', '\u2807', '\u280F'];
    const spinnerColors = ['\x1B[35m', '\x1B[34m', '\x1B[36m', '\x1B[32m', '\x1B[33m', '\x1B[31m'];
